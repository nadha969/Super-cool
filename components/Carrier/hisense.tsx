...
// Fetch Hisense products
async function getProducts(): Promise<Product[]> {
  const res = await fetch(`${BASE_URL}/api/brands/hisense`, {
    cache: "no-store",
  });
...
export default async function Hisense() {
    ...
    <div>
      ...
      <main className="pt-30 flex-1">
        <h1 className="text-center text-3xl font-semibold bg-gray-100 py-10">
          Hisense Air Conditioners Collection
        </h1>
      </main>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="pb-10">
          <p className="text-center text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
            Hisense Air Conditioners provide powerful cooling, energy efficiency,
            and smart technology, delivering reliable comfort for homes,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {categoryData.map((cat, index) => (
            ...
            >
              {/* Glow Background */}
              <div className="absolute inset-0 bg-gradient-to-br from-cyan-50 via-transparent to-blue-100 opacity-0 group-hover:opacity-100 transition duration-500" />
              ...
              {/* Title */}
              <h3 className="relative z-10 text-center font-bold text-lg uppercase text-gray-800 group-hover:text-[#00AEEF] transition-colors duration-300">
                {cat.name}
              </h3>

              {/* Bottom Line */}
              <div className="relative z-10 mt-4 h-1 w-0 bg-[#00AEEF] mx-auto rounded-full group-hover:w-20 transition-all duration-500"></div>
            </Link>
          ))}
        </div>
      </section>
