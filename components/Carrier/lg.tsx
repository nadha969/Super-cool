...
async function getProducts(): Promise<Product[]> {
  const res = await fetch(`${BASE_URL}/api/brands/lg`, {
    cache: "no-store",
  });
    ...
    <div>
      ...
      <main className="pt-30 flex-1">
        <h1 className="text-center text-3xl font-semibold bg-gray-100 py-10">
          LG Air Conditioners Collection
        </h1>
      </main>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="pb-10">
          <p className="text-center text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
            LG Air Conditioners combine innovative cooling technology, energy
            efficiency, and smart features to provide exceptional comfort for
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {categoryData.map((cat, index) => (
            ...
            >
              {/* Glow Background */}
              <div className="absolute inset-0 bg-gradient-to-br from-red-50 via-transparent to-pink-100 opacity-0 group-hover:opacity-100 transition duration-500" />
              ...
              {/* Title */}
              <h3 className="relative z-10 text-center font-bold text-lg uppercase text-gray-800 group-hover:text-[#A50034] transition-colors duration-300">
                {cat.name}
              </h3>

              {/* Bottom Line Animation */}
              <div className="relative z-10 mt-4 h-1 w-0 bg-[#A50034] mx-auto rounded-full group-hover:w-20 transition-all duration-500"></div>
            </Link>
          ))}
        </div>
      </section>
